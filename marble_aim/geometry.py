...
def estimate_ball_radius(
    ...
    *,
    block_ratio: float = 0.202,
    board_width_ratio: float = 0.032,
    fallback: float = 7.0,
) -> float:
    sizes = [
...
def survival_priority_score(
    board: Rect,
    obstacles: list[Obstacle],
    hit_counts: dict[int, int],
    destroyed_indices: set[int] | None = None,
) -> float:
    if not hit_counts or board.width <= 0:
        return 0.0
    row_step = board.width / 6.0
    score = 0.0
    ...
    for index, obstacle in enumerate(obstacles):
        row_number = (
            (float(obstacle.rect.center[1]) - board.top) / row_step + 0.5
        )
        row_number = max(1.0, min(8.0, row_number))
        weight = 0.0 if row_number < 1.5 else 50.0 * 6.0 ** (row_number - 2.0)
    ...
    for obstacle_index, hits in hit_counts.items():
        ...
        weight = weights[obstacle_index]
        durability = obstacle.durability
        if destroyed_indices is None:
            utility = min(1.0, 0.30 + min(hits, 8) * 0.10)
        elif obstacle_index in destroyed_indices:
            utility = 1.25
        elif durability is not None:
            damage_fraction = min(1.0, hits / max(1, durability))
            utility = 0.05 + damage_fraction * 0.45
        else:
            utility = min(0.30, min(hits, 8) * 0.04)
        score += weight * utility

    if (
        destroyed_indices is not None
        and critical_indices
        and critical_indices.issubset(destroyed_indices)
    ):
        score += critical_weight * 1.5
    return score
    ...
    *,
    ...
    for obstacle_index, hits in hit_counts.items():
        ...
        if hits >= durability:
            ...
            destroyed_indices.add(obstacle_index)
    ...
    result.score = (
        ...
        + survival_priority_score(
            ...
            obstacles,
            hit_counts,
            destroyed_indices if has_durability else None,
        )
        - grazing * 0.25
...
def simulate_volley_trajectory(
    ...
    **simulation_options: float | int | bool,
    ...
    aggregate_hit_counts: dict[int, int] = {}
    ...
    while balls_left > 0 and active_indices:
        ...
        for index, hits in hit_counts.items():
            ...
            aggregate_hit_counts[index] = (
                aggregate_hit_counts.get(index, 0) + hits * batch
            )
...
def search_recommendations(
    ...
    **simulation_options: float | int,
    ...
    obstacles = list(obstacles)
    angle_min = max(-89.0, float(angle_min))
    angle_max = min(89.0, float(angle_max))
    if angle_min > angle_max:
        angle_min, angle_max = angle_max, angle_min
    coarse_step = max(0.5, float(coarse_step))
    fine_step = max(0.05, min(coarse_step, float(fine_step)))
