...
class PhysicsConfig:
    ...
    ball_radius_to_block_ratio: float = 0.202
    ball_radius_to_board_width_ratio: float = 0.032
    max_collisions: int = 8
    max_distance_factor: float = 8.0
    ...
    def __post_init__(self) -> None:
        if abs(self.ball_radius_to_block_ratio - 0.215) < 1e-6:
            self.ball_radius_to_block_ratio = 0.202
        if abs(self.ball_radius_to_board_width_ratio - 0.0345) < 1e-6:
            self.ball_radius_to_board_width_ratio = 0.032
        self.max_collisions = max(5, min(10, int(self.max_collisions)))
        self.angle_min_deg = max(-89.0, min(89.0, float(self.angle_min_deg)))
        self.angle_max_deg = max(-89.0, min(89.0, float(self.angle_max_deg)))
        if self.angle_min_deg > self.angle_max_deg:
            self.angle_min_deg, self.angle_max_deg = (
                self.angle_max_deg,
                self.angle_min_deg,
            )
        self.coarse_step_deg = max(0.5, float(self.coarse_step_deg))
        self.fine_step_deg = max(
            0.05,
            min(
                self.coarse_step_deg,
                float(self.fine_step_deg),
            ),
        )
        self.recommendation_count = 1
        self.volley_count = max(1, min(200, int(self.volley_count)))
        self.advanced_simulation = bool(self.advanced_simulation)
...
class OverlayConfig:
    ...
    def __post_init__(self) -> None:
        self.trajectory_mode = str(self.trajectory_mode).upper()
        if self.trajectory_mode not in {"A", "B", "C", "D"}:
            self.trajectory_mode = "A"
