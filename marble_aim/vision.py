...
_REFERENCE_FULLSCREEN_SIZE = (3840.0, 2160.0)
_REFERENCE_COLLISION_BOARD = Rect(1433.0, 323.0, 2403.0, 1620.0)
_REFERENCE_RECOGNITION_REGION = Rect(1426.0, 319.0, 2422.0, 1621.0)
...
class DetectionResult:
    ...
    volley_count: int | None = None
    volley_confidence: float = 0.0
...
def _fixed_fullscreen_regions(
    ...
    if abs(width / height - 16 / 9) > 0.01:
        return None
    reference_width, reference_height = _REFERENCE_FULLSCREEN_SIZE
    ...
    return (
        scaled(_REFERENCE_COLLISION_BOARD),
        scaled(_REFERENCE_RECOGNITION_REGION),
    )
...
def _recognition_region_from_pale(pale: Image, board: Rect) -> Rect:
    """Expand inner collision edges to the surrounding visible pool frame."""
    height, width = pale.shape
    board_width = max(1.0, board.width)
    board_height = max(1.0, board.height)
    y0 = max(0, round(board.top + board_height * 0.04))
    y1 = min(height, round(board.bottom - board_height * 0.04))
    if y1 <= y0:
        return board
    column_counts = (pale[y0:y1] > 0).sum(axis=0)
    column_runs = _runs(
        np.flatnonzero(column_counts >= max(2, (y1 - y0) * 0.50))
    )
    ...
    def nearest_run(
        ...
        nearby = [
            run
            for run in runs
            if run[0] - radius <= coordinate <= run[1] + radius
        ]
        if not nearby:
            return None
        return min(
            ...
            key=lambda run: (
                0.0
                if run[0] <= coordinate <= run[1]
                else min(abs(coordinate - run[0]), abs(coordinate - run[1]))
            ),
        )

    left_run = nearest_run(column_runs, board.left, board_width * 0.04)
    right_run = nearest_run(column_runs, board.right, board_width * 0.04)
    left = float(left_run[0]) if left_run is not None else board.left
    right = float(right_run[1]) if right_run is not None else board.right

    x0 = max(0, round(left))
    x1 = min(width, round(right) + 1)
    if x1 <= x0:
        return board
    row_counts = (pale[:, x0:x1] > 0).sum(axis=1)
    row_runs = _runs(
        np.flatnonzero(row_counts >= max(2, (x1 - x0) * 0.55))
    )
    top_run = nearest_run(row_runs, board.top, board_height * 0.03)
    bottom_run = nearest_run(row_runs, board.bottom, board_height * 0.03)
    top = float(top_run[0]) if top_run is not None else board.top
    bottom = float(bottom_run[1]) if bottom_run is not None else board.bottom
...
def _detect_grid_blocks(
    ...
    *,
    ...
    region = recognition_region or board
    cv2.rectangle(
        roi,
        (max(0, round(region.left)), max(0, round(region.top))),
        (min(width - 1, round(region.right)), min(height - 1, round(region.bottom))),
        255,
        -1,
    )
    ...
    if fixed_grid:
        ...
        dark_map = (hsv[:, :, 2] < 105) & (hsv[:, :, 1] > 45)
        cell_size = grid_step * 0.96
        anchor_x = board.left + grid_step * 0.03
        anchor_y = board.top + grid_step * 1.083
        rows = max(
            1,
            math.floor(
                (board.bottom - anchor_y - cell_size * 0.50) / grid_step
            )
            + 1,
        )
        obstacles: list[Obstacle] = []
        for row in range(rows):
            for column in range(6):
                left = round(anchor_x + column * grid_step)
                top = round(anchor_y + row * grid_step)
                right = min(width - 1, round(left + cell_size - 1))
                bottom = min(height - 1, round(top + cell_size - 1))
                if right <= left or bottom <= top:
                    continue
                edges = edge_map[top : bottom + 1, left : right + 1] > 0
                dark = dark_map[top : bottom + 1, left : right + 1]
                edge_density = float(np.mean(edges))
                dark_density = float(np.mean(dark))
                band = max(3, round(cell_size * 0.08))
                border = np.zeros_like(edges)
                border[:band] = True
                border[-band:] = True
                border[:, :band] = True
                border[:, -band:] = True
                border_edge_density = float(np.mean(edges[border]))
                border_dark_density = float(np.mean(dark[border]))
                if not (
                    edge_density >= 0.080
                    and dark_density >= 0.032
                    and border_edge_density >= 0.090
                    and border_dark_density >= 0.040
                ):
                    continue
    ...
    if seed_obstacles:
        ...
        for obstacle in seed_obstacles:
            if obstacle.rect.width < nominal_cell * 0.45:
                continue
            column = round(
                (obstacle.rect.center[0] - board.left - step_x / 2) / step_x
            )
            if obstacle.rect.width >= cell_width * 0.72:
                x_phases.append(obstacle.rect.left - column * step_x)
            else:
                x_phases.append(
                    board.left + (step_x - cell_width) / 2
                )
        ...
        for obstacle in seed_obstacles:
            if (
                obstacle.rect.width < nominal_cell * 0.45
                or obstacle.rect.height < nominal_cell * 0.40
            ):
                continue
            column = round(
...
class BoardDetector:
    def __init__(self, config: VisionConfig, *, recognize_numbers: bool = False):
        self.config = config
    ...
    def detect(self, bgr: Image, *, debug_masks: bool = False) -> DetectionResult:
        hsv = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
        height, width = hsv.shape[:2]
        fixed_regions = _fixed_fullscreen_regions(width, height)
        if fixed_regions is not None:
            collision_board, recognition_region = fixed_regions
            ...
            recognition_region = (
                _recognition_region_from_pale(pale_mask, collision_board)
        ...
        obstacles: list[Obstacle] = []
        if collision_board is not None:
            obstacles, block_mask = _detect_grid_blocks(
                bgr,
                hsv,
                collision_board,
                recognition_region,
                fixed_grid=fixed_regions is not None,
            )
        else:
        ...
        volley_count = None
        volley_confidence = 0.0
        if self.recognize_numbers and board is not None:
            obstacles = recognize_obstacle_durabilities(bgr, obstacles)
            launch_x = aim_line[0][0] if aim_line is not None else None
            volley = recognize_volley_count(bgr, board, launch_x)
            if volley is not None:
                volley_count = volley.value
                volley_confidence = volley.confidence
        ...
        return DetectionResult(
            ...
            recognition_region=recognition_region or board,
...
class TemporalDetector:
    ...
    def detect(self, bgr: Image, *, debug_masks: bool = False) -> DetectionResult:
        ...
        valid_regions = [
            ...
            if result.recognition_region is not None
        ]
        recognition_region = (
            Rect(
                float(np.median([item.left for item in valid_regions])),
                float(np.median([item.top for item in valid_regions])),
                float(np.median([item.right for item in valid_regions])),
                float(np.median([item.bottom for item in valid_regions])),
            )
            if valid_regions
            else board
        )
        ...
        for group in groups:
            ...
            durabilities = [
                ...
                and item.durability_confidence >= 0.25
            ]
            durability = (
                int(round(float(np.median(durabilities))))
                if len(durabilities) >= required
                else None
            )
            durability_confidence = (
                float(np.mean(
                    [
                        item.durability_confidence
                        for item in group
                        if item.durability is not None
                    ]
                ))
        ...
        volley_counts = [
            ...
            if item.volley_count is not None and item.volley_confidence >= 0.25
        ]
        return DetectionResult(
            board=board,
            ...
            volley_count=(
                int(round(float(np.median(volley_counts))))
                if len(volley_counts) >= required
                else current.volley_count
            ),
            volley_confidence=(
                float(np.mean(
                    [
                        item.volley_confidence
                        for item in self.history
                        if item.volley_count is not None
                    ]
                ))
                if volley_counts
                else current.volley_confidence
            ),
        )
...
def render_debug(bgr: Image, result: DetectionResult) -> Image:
    output = bgr.copy()
    if result.recognition_region:
        region = result.recognition_region
        cv2.rectangle(
            output,
            (round(region.left), round(region.top)),
            (round(region.right), round(region.bottom)),
            (255, 255, 0),
            2,
        )
        cv2.putText(
            output,
            "VISION ROI",
            (round(region.left), max(14, round(region.top) - 5)),
    ...
    for obstacle in result.obstacles:
        ...
        cv2.putText(
            output,
            (
                f"BLOCK {obstacle.identifier} HP {obstacle.durability}"
                if obstacle.durability is not None
                else f"BLOCK {obstacle.identifier}"
            ),
