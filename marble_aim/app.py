...
class ApplicationController(QObject):
    def __init__(
        ...
        *,
        ...
        self.detector = TemporalDetector(
            BoardDetector(
                config.vision,
                recognize_numbers=config.physics.advanced_simulation,
            ),
        ...
        self.refresh_button = RefreshButton(
            config.overlay.trajectory_mode,
            config.physics.advanced_simulation,
        )
        ...
        self.refresh_button.mode_cycle_requested.connect(self.cycle_trajectory_mode)
        self.refresh_button.advanced_toggle_requested.connect(
            self.toggle_advanced_simulation
        )
        ...
        self.overlay_capture_geometry: tuple[int, int, int, int] | None = None
    ...
    def start(self) -> None:
        ...
        self._position_overlay(geometry)
    ...
    def _position_overlay(self, geometry: WindowGeometry) -> None:
        capture_geometry = (
            geometry.left,
            geometry.top,
            geometry.width,
        ...
        if capture_geometry != self.overlay_capture_geometry:
            set_native_window_geometry(self.overlay, geometry)
            self.overlay.set_capture_size(geometry.width, geometry.height)
            self.overlay_capture_geometry = capture_geometry
        set_native_window_position(
            self.refresh_button,
            geometry.left + 8,
            geometry.top + 8,
    ...
    def _physics_options(self) -> dict[str, float | int]:
        ...
        return {
            ...
            "max_collisions": max(5, min(10, physics.max_collisions)),
            "max_distance": (
    ...
    def _make_scene(self, detection: DetectionResult) -> SceneSnapshot | None:
        ...
        return SceneSnapshot(
            ...
            ball_radius,
            (
                detection.volley_count
                if (
                    self.config.physics.advanced_simulation
                    and detection.volley_count is not None
                    and detection.volley_confidence >= 0.25
                )
                else self.config.physics.volley_count
            ),
            (
                detection.volley_confidence
                if self.config.physics.advanced_simulation
                else 0.0
            ),
        )
    ...
    def _scene_signature(self, scene: SceneSnapshot) -> tuple[object, ...]:
        return (
            ...
            self.config.physics.advanced_simulation,
            scene.volley_count if self.config.physics.advanced_simulation else 1,
            tuple(
                obstacle.durability
    ...
    def _detect(self, frame: np.ndarray) -> None:
        ...
        self.status = (
            ...
            + (
                f"高级模拟 {scene.volley_count} 球/"
                f"{sum(item.durability is not None for item in scene.obstacles)} 个血量｜"
                if self.config.physics.advanced_simulation
                else "标准模拟｜"
            )
            + f"自动规划最佳路线＋手控轨迹｜置信度 {detection.confidence:.0%}｜"
    ...
    def _check_scene_transition(self, frame: np.ndarray) -> str:
        ...
        self.status = (
            ...
            f"球半径 {scene.ball_radius:.1f}px｜正在规划最佳路线"
        )
        return "relocked"
    ...
    def _search_for_scene(self, scene: SceneSnapshot) -> list[Trajectory]:
        ...
        return search_recommendations(
            ...
            count=physics.recommendation_count,
            separation=physics.recommendation_separation_deg,
            volley_count=(
                scene.volley_count
                if physics.advanced_simulation
                else 1
            ),
            **options,
        )

    def _recommendation_key(self, scene: SceneSnapshot) -> tuple[object, ...]:
        physics = self.config.physics
        return (
            *self._scene_signature(scene),
            round(scene.origin[0] / 2.0),
            round(scene.origin[1] / 2.0),
            physics.max_collisions,
            round(physics.max_distance_factor, 2),
            round(physics.angle_min_deg, 2),
            round(physics.angle_max_deg, 2),
            round(physics.coarse_step_deg, 2),
            round(physics.fine_step_deg, 2),
        )

    def _collect_recommendations(self) -> None:
        future = self.recommendation_future
        signature = self.recommendation_future_signature
        if future is None or signature is None or not future.done():
            return
        ...
        try:
            ...
            self.recommendations = []
            self.status = f"推荐轨迹计算失败：{error}"
            return
        self.recommendation_cache[signature] = recommendations
        scene = self.latest_scene
        if scene is not None and signature == self._recommendation_key(scene):
            self.recommendations = recommendations
            self.recommendation_signature = signature

    def _request_recommendations(self, scene: SceneSnapshot) -> None:
        signature = self._recommendation_key(scene)
        if signature == self.recommendation_signature:
            return
        cached = self.recommendation_cache.get(signature)
        ...
        if not scene.obstacles:
            ...
            return
        self.recommendations = []
        self.recommendation_future_signature = signature
        self.recommendation_future = self.executor.submit(
            self._search_for_scene,
            scene,
        )
    ...
    def refresh_scene(self) -> None:
        ...
        self.detector = TemporalDetector(
            BoardDetector(
                self.config.vision,
                recognize_numbers=self.config.physics.advanced_simulation,
            ),
    ...
    def cycle_trajectory_mode(self) -> None:
        modes = ("A", "B", "C", "D")
        current = self.config.overlay.trajectory_mode
        index = modes.index(current) if current in modes else 0
        mode = modes[(index + 1) % len(modes)]
        self.config.overlay.trajectory_mode = mode
        self.refresh_button.set_trajectory_mode(mode)
        descriptions = {
            "A": "绿落点＋紫预测",
            "B": "仅绿落点",
            "C": "绿落点＋绿预测＋紫预测",
        ...
        self.status = f"轨迹显示模式 {mode}：{descriptions[mode]}"
        self.config.save(self.config_path)
        self.overlay.update()
    ...
    def toggle_advanced_simulation(self) -> None:
        enabled = not self.config.physics.advanced_simulation
        self.config.physics.advanced_simulation = enabled
        self.refresh_button.set_advanced_simulation(enabled)
        self.config.save(self.config_path)
        self.refresh_scene()
        self.status = (
    ...
    def open_settings(self) -> None:
        ...
        advanced_before = self.config.physics.advanced_simulation
        if dialog.exec():
            dialog.apply()
            self.debug_view = dialog.debug.isChecked()
            self.overlay.debug_view = self.debug_view
            self.refresh_button.set_advanced_simulation(
                self.config.physics.advanced_simulation
            )
            self.recommendation_signature = None
            self.stable_scene_signature = None
            ...
            if (
                advanced_before != self.config.physics.advanced_simulation
                or self.config.physics.advanced_simulation
            ):
                self.refresh_scene()
        if was_visible:
            self.overlay.show()
    ...
    def tick(self) -> None:
        ...
        if self.calibration_deadline is not None and self.latest_scene is not None:
            ...
            if self.scene_locked:
                ...
                self.last_detection_at = now
        scene = self.latest_scene
        if scene is not None and self.scene_locked:
            self._request_recommendations(scene)
        self.overlay.update_scene(
            self.current_trajectory,
            self.recommendations if self.fast_aim_direction is not None else [],
            scene.board if scene else None,
            list(scene.obstacles) if scene else [],
            self.status,
            scene.ball_radius if scene else None,
            (
                self.latest_detection.recognition_region
