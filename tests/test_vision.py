...
def test_fullscreen_regions_scale_from_fixed_3840_reference():
    regions = _fixed_fullscreen_regions(3840, 2160)
    assert regions is not None
    board, recognition_region = regions
    assert board == Rect(1433, 323, 2403, 1620)
    assert recognition_region == Rect(1426, 319, 2422, 1621)

    scaled = _fixed_fullscreen_regions(1920, 1080)
    assert scaled is not None
    scaled_board, _ = scaled
    assert scaled_board == Rect(716.5, 161.5, 1201.5, 810)


def test_fixed_fullscreen_grid_rejects_overlay_only_cells():
    image = np.full((1080, 1920, 3), hsv_to_bgr(100, 210, 235), dtype=np.uint8)
    regions = _fixed_fullscreen_regions(1920, 1080)
    assert regions is not None
    board, _ = regions
    step = board.width / 6
    cell_size = step * 0.96
    anchor_x = board.left + step * 0.03
    anchor_y = board.top + step * 1.083
    dark = hsv_to_bgr(100, 170, 80)
    orange = hsv_to_bgr(20, 220, 235)
    for column in (0, 5):
        left = round(anchor_x + column * step)
        top = round(anchor_y)
        right = round(left + cell_size - 1)
        bottom = round(top + cell_size - 1)
        cv2.rectangle(image, (left, top), (right, bottom), dark, 8)
        cv2.rectangle(
            image,
            (left + 6, top + 6),
            (right - 6, bottom - 12),
            hsv_to_bgr(100, 210, 235),
            -1,
        )
        cv2.rectangle(
            image,
    ...
    false_left = round(anchor_x)
    false_top = round(anchor_y + step * 3)
    false_right = round(false_left + cell_size - 1)
    false_bottom = round(false_top + cell_size - 1)
    cv2.rectangle(
        image,
        (false_left, false_top),
        (false_right, false_bottom),
        hsv_to_bgr(60, 220, 235),
        4,
    )
...
def test_temporal_detector_stabilizes_durability_and_volley_count():
    board = Rect(20, 20, 220, 330)
    results = [
        DetectionResult(
            board=board,
            obstacles=[
            ...
            launch_origin=(120, 330),
            aim_line=((120, 330), (130, 100)),
            volley_count=count,
            volley_confidence=0.8,
        )
        for count in (31, 31, 13)
    ]

    class DetectorStub:
        def detect(self, bgr, *, debug_masks=False):
            return results.pop(0)

    detector = TemporalDetector(DetectorStub(), frames=3)
    detector.detect(synthetic_board())
    detector.detect(synthetic_board())
    result = detector.detect(synthetic_board())

    assert result.obstacles[0].durability == 48
    assert result.volley_count == 31
...
def test_collision_frame_grid_blocks_and_aim_line():
    ...
    assert result.board is not None
    assert result.recognition_region is not None
    ...
    assert abs(result.recognition_region.left - 240) <= 2
    assert abs(result.recognition_region.right - 660) <= 2
    assert abs(result.recognition_region.top - 90) <= 2
    assert abs(result.recognition_region.bottom - 510) <= 2
...
def test_partial_block_artwork_anchors_full_grid_cells():
    image = np.full((600, 900, 3), hsv_to_bgr(100, 210, 235), dtype=np.uint8)
    pale = hsv_to_bgr(95, 35, 245)
    dark = hsv_to_bgr(100, 170, 80)
    orange = hsv_to_bgr(20, 220, 235)
    ...
    cv2.rectangle(image, (650, 90), (660, 510), pale, -1)
    for x in (253, 587):
        cv2.rectangle(image, (x, 180), (x + 61, 241), dark, 3)
        cv2.rectangle(
            image,
            (x + 4, 184),
            (x + 57, 231),
        ...
        cv2.rectangle(image, (x + 16, 190), (x + 46, 221), orange, -1)

    result = BoardDetector(VisionConfig()).detect(image)

    assert result.collision_frame_detected
    assert len(result.obstacles) == 2
    assert result.obstacles[0].rect.left < 260
    assert result.obstacles[1].rect.right > 640
